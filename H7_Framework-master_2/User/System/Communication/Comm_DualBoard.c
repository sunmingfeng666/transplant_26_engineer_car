...
// 反馈帧完整性校验（通信层职责：SOF/TAIL/版本/类型/checksum）。
static bool Feedback_Frame_Is_Valid(const B2B_Engineer_Feedback_Frame_t *frame)
{
    if (frame == NULL || frame->sof != DUALBOARD_SOF || frame->tail != DUALBOARD_TAIL) return false;
    if (frame->version != DUALBOARD_ENGINEER_VERSION ||
        frame->type != DUALBOARD_FRAME_TYPE_ENGINEER_FEEDBACK) return false;
    return frame->checksum == Frame_Checksum(frame, ENGINEER_FEEDBACK_CHECKSUM_INDEX);
}

// 反馈帧收件箱：通信层收到并通过校验的原始帧暂存于此，等 App 命令层取走解析。
// 单缓冲、后到覆盖先到，pending 最后置位保证数据完整。
static uint8_t  fb_inbox_buf[DUALBOARD_MAX_PAYLOAD];
static uint16_t fb_inbox_len = 0;
static volatile uint8_t fb_inbox_pending = 0;

static void Inbox_Store(const uint8_t *buf, uint16_t len)
{
    if (buf == NULL || len == 0U || len > sizeof(fb_inbox_buf)) return;
    memcpy(fb_inbox_buf, buf, len);
    fb_inbox_len = len;
    fb_inbox_pending = 1U;
}
...
uint8_t DualBoard_Take_Feedback_Frame(uint8_t *out, uint16_t out_cap, uint16_t *out_len)
{
    ...
    __disable_irq();
    uint8_t got = 0U;
    if (fb_inbox_pending && fb_inbox_len <= out_cap) {
        memcpy(out, fb_inbox_buf, fb_inbox_len);
        *out_len = fb_inbox_len;
        fb_inbox_pending = 0U;
        got = 1U;
    }
                                         ...
                                         const B2B_Engineer_Feedback_t *feedback)
{
    ...
    engineer_feedback_tx_frame.checksum = Frame_Checksum(&engineer_feedback_tx_frame,
                                                          ENGINEER_FEEDBACK_CHECKSUM_INDEX);
    return (HAL_UART_Transmit(huart, (uint8_t *)&engineer_feedback_tx_frame,
                              sizeof(engineer_feedback_tx_frame), 2U) == HAL_OK) ? 0U : 2U;
}
...
void DualBoard_UART_Rx(uint8_t *Buff, uint16_t Size)
{
    ...
    // 本板(决策板)只消费整车反馈帧；命令帧不该收到，其它类型丢弃。
    if (Size == sizeof(B2B_Engineer_Feedback_Frame_t) &&
        Buff[3] == DUALBOARD_FRAME_TYPE_ENGINEER_FEEDBACK) {
        if (Feedback_Frame_Is_Valid((const B2B_Engineer_Feedback_Frame_t *)Buff)) {
